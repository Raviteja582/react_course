...
/* These are use to perform actions of a specific action type.  */
...
export const postComment = (dishId, rating=1, comment) => (dispatch) => {
...
export const postFeedback = (firstname, lastname, telnum, email, agree, contactType, message, id) => (dispatch) => {
  ...
  .then(response => response.json())
  .then(response => alert("Thank u for your Feedback\n" + JSON.stringify(response)))
  .catch(error =>  { console.log('post comments', error.message); alert('Your comment could not be posted\nError: '+error.message); });
};



/* Funtion returning a funtion as same as redux thunk */
...
/* Favorites feature ActionCreators type */
